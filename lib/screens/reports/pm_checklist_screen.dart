...
class _PMChecklistScreenState extends State<PMChecklistScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final checklistRes = await _supabase
          ...
          .select(
        '*, equipment:m_equipment!inner(name, m_area!inner(m_zone!inner(kitchen_id)))',
      )
          .eq('status', true)
              ...
              children: [
                ...
                Row(
                  children: [
                    ChoiceChip(
                      ...
                      onSelected: (v) =>
                          setDialogState(() => exportMode = 'month'),
                    ),
                    ...
                    ChoiceChip(
                      ...
                      backgroundColor: surface,
                      onSelected: (v) =>
                          setDialogState(() => exportMode = 'machine'),
                    ),
                  ],
                ),
                ...
                if (exportMode == 'month') ...[
                  ...
                  RawAutocomplete<Map<String, dynamic>>(
                    ...
                    onSelected: (sel) {
                      setDialogState(() => selectedMachineId = sel['id']);
                      focusNode.unfocus();
                    },
                    fieldViewBuilder: (ctx, ctrl, fNode, onSub) =>
                        TextFormField(
                          ...
                          decoration: minimalDialogDecor("Search Machine")
                              .copyWith(
  ...
  Future<void> _executeExport(
    ...
    if (targetKitchenId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No active kitchen available!'),
    ...
    if (mode == 'machine' && machineId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select a machine first.')),
      );
      return;
    }
    ...
    try {
      ...
      final url = Uri.parse(
        '${ApiConstants.pythonApiBaseUrl}/reports/pm-checklists$queryParams',
      );
      ...
      if (response.statusCode == 200) {
        ...
        Directory? saveDir = Platform.isAndroid
            ...
            : Directory(
          '${(await getApplicationDocumentsDirectory()).path}/PM Checklists',
        );
  ...
  Widget build(BuildContext context) {
    final filteredChecklists = _checklists.where((c) {
      final machineName = (c['equipment']?['name'] ?? '')
          .toString()
          .toLowerCase();
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () async {
            final result = await Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => const CreatePMChecklistScreen(),
              ),
            );
            if (result == true) _fetchData();
          },
...
class CreatePMChecklistScreen extends StatefulWidget {
  ...
  @override
  State<CreatePMChecklistScreen> createState() =>
      _CreatePMChecklistScreenState();
}
...
class _CreatePMChecklistScreenState extends State<CreatePMChecklistScreen> {
  ...
  String? _getActiveKitchenId() {
    ...
    String targetKitchenId = ticketProv.kitchenFilter;
    if (targetKitchenId == 'ALL' || targetKitchenId.isEmpty) {
      return authProv.assignedKitchens.isNotEmpty
  ...
  Future<void> _loadExistingData() async {
    ...
    _selectedDate =
        DateTime.tryParse(widget.existingChecklist!['date']) ?? DateTime.now();
    _selectedFrequency = widget.existingChecklist!['frequency'] ?? 'Monthly';
    _machineSearchCtrl.text =
        widget.existingChecklist!['equipment']?['name'] ?? 'Unknown Machine';
        ...
        for (var act in res) {
          _activities.add({
            ...
            'condition': TextEditingController(
              text: act['standard_condition'] ?? '',
            ),
            'status': TextEditingController(
              text: act['condition_status'] ?? 'OK',
            ),
            'observation': TextEditingController(
              text: act['observation'] ?? '',
            ),
          });
        }
      }
  ...
  Future<void> _saveChecklist() async {
    ...
    for (var a in _activities) {
      if (a['activity']!.text.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Please fill out all activity names.")),
        );
        return;
      }
    }
    ...
    try {
      ...
      if (_activities.isNotEmpty) {
        final List<Map<String, dynamic>> activitiesToInsert = _activities
            .map(
              (a) => {
    ...
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: Text(
            isEditing ? "Edit PM Checklist" : "New PM Checklist",
            style: GoogleFonts.inter(
              fontWeight: FontWeight.w700,
              letterSpacing: -0.5,
            ),
                  ...
                  return Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 12),
                        TextField(
                          controller: activity['condition'],
                          style: GoogleFonts.inter(fontSize: 14),
                          decoration: _minimalDecor("Standard Condition"),
                        ),
                        const SizedBox(height: 12),
                        DropdownButtonFormField<String>(
                          ...
                          items: _statusOptions
                              ...
                              .toList(),
                          onChanged: (v) =>
                              setState(() => activity['status']!.text = v!),
                        ),
                        const SizedBox(height: 12),
                        TextField(
                          controller: activity['observation'],
                          style: GoogleFonts.inter(fontSize: 14),
                          decoration: _minimalDecor("Observation"),
                        ),
                      ],
                    ),
                  );
            ...
            child: ElevatedButton(
              ...
              child: _isSaving
                ...
                child: CircularProgressIndicator(
                  ...
                  : Text(
                isEditing ? "Save Changes" : "Save Checklist",
                style: GoogleFonts.inter(
                  fontWeight: FontWeight.w600,
                  fontSize: 15,
                  color: Colors.white,
