...
class _DGLogListScreenState extends State<DGLogListScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (isCurrentMonth) {
        final todayExists = fetchedRecords.any(
          (r) => r['log_date'] == todayStr,
        );
        if (!todayExists) {
  ...
  Future<void> _executeExport(int month, int year, String format) async {
    ...
    try {
      final url = Uri.parse(
        '${ApiConstants.pythonApiBaseUrl}/reports/dg-set?kitchen_id=$targetKitchenId&month=$month&year=$year&format=$format',
      );
      final response = await http.get(url);

      if (response.statusCode == 200) {
        Directory? saveDir = Platform.isAndroid
            ? Directory('/storage/emulated/0/Download/DG Reports')
            : Directory(
                '${(await getApplicationDocumentsDirectory()).path}/DG Reports',
              );
        if (!await saveDir.exists()) await saveDir.create(recursive: true);

        final expectedFilename =
            'MT14_DG_Report_${year}_${month.toString().padLeft(2, '0')}.$format';
        ...
        children: [
          ...
          Expanded(
            child: _isLoading
                ...
                ? Center(
                    child: Text(
                      "No DG logs found for ${_months[_selectedMonth - 1]} $_selectedYear.",
                          ...
                          child: InkWell(
                            ...
                            onTap: () async {
                              final result = await Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (_) =>
                                      DGLogFormScreen(existingRecord: item),
                                ),
                              );
                              ...
                              child: Row(
                                children: [
                                  Container(
                                    ...
                                    child: Icon(
                                      isVerified ? Icons.verified : Icons.bolt,
                                      color: isVerified
                                          ? Colors.green
                                          : primary,
                                      size: 24,
...
class _DGLogFormScreenState extends State<DGLogFormScreen> {
  ...
  @override
  void dispose() {
    _coolantTempCtrl.dispose();
    _engineOilTempCtrl.dispose();
    _batteryVoltageCtrl.dispose();
    _dgFrequencyCtrl.dispose();
    _engineOilPressureCtrl.dispose();
    _runningHoursCtrl.dispose();
    _totalKwhCtrl.dispose();
    _dieselFillCtrl.dispose();
    _dieselConsCtrl.dispose();
    _dieselStockCtrl.dispose();
  ...
  Future<void> _initData() async {
    ...
    if (r['signature'] != null &&
        !r['signature'].toString().startsWith('http')) {
      _existingSignatureUrl = _supabase.storage
          .from('ticket-media')
          .getPublicUrl(r['signature']);
    } else {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              _isVerified ? "Log Verified & Locked!" : "Log Saved!",
            ),
        ...
        children: [
          Expanded(
            child: TextField(
              ...
              decoration: _decor(l1, suffix: s1),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: TextField(
              ...
              decoration: _decor(l2, suffix: s2),
            ),
          ),
        ],
      ),
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            _isReadOnly
                ? "View Confirmed Log"
                : (isBlank ? "Fill Log Entry" : "Edit Log Entry"),
            style: GoogleFonts.inter(
              fontWeight: FontWeight.w700,
              letterSpacing: -0.5,
            ),
            ...
            children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          "This log has been verified and is permanently locked.",
                          style: GoogleFonts.inter(
              ...
              _buildSectionCard(
                ...
                children: [
                  TextFormField(
                    ...
                    decoration: _decor("Assigned Facility/Kitchen").copyWith(
                  ...
                  TextFormField(
                    initialValue:
                        "${_logDate.day.toString().padLeft(2, '0')}-${_logDate.month.toString().padLeft(2, '0')}-${_logDate.year}",
                    enabled: false,
                    style: GoogleFonts.inter(
                      fontSize: 14,
                      fontWeight: FontWeight.w600,
                    ...
                    decoration: _decor("Log Entry Date").copyWith(
                      prefixIcon: const Icon(
              ...
              _buildSectionCard(
                ...
                children: [
                  ...
                  TextField(
                    controller: _totalKwhCtrl,
                    enabled: !_isReadOnly,
                    keyboardType: const TextInputType.numberWithOptions(
                      decimal: true,
                    ),
                    decoration: _decor("Total Meter KWH", suffix: "kWH"),
                  ),
                ],
              ),
              ...
              _buildSectionCard(
                ...
                children: [
                  ...
                  TextField(
                    controller: _dieselStockCtrl,
                    enabled: !_isReadOnly,
                    keyboardType: const TextInputType.numberWithOptions(
                      decimal: true,
                    ),
                    decoration: _decor("Diesel Stock Status", suffix: "%"),
                  ),
                ],
              ),
              ...
              _buildSectionCard(
                ...
                children: [
                  TextField(
                    ...
                    maxLines: 3,
                    decoration: _decor(
                      "Enter observations or write standard notes here...",
                    ),
                  ),
              ...
              if (!_isReadOnly)
                Container(
                  ...
                  child: CheckboxListTile(
                    ...
                    subtitle: Text(
                      "Once verified, this record cannot be edited again.",
                      style: GoogleFonts.inter(
                        fontSize: 12,
                        color: Colors.grey.shade700,
                      ),
